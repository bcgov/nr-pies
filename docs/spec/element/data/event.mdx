...
- All Date and DateTime values shall be represented in RFC 3339 format.
  - The `date` format is represented as `YYYY-MM-DD`.
    - The `date` format must contain leading padding zeroes where necessary (such as `2024-12-01`).
  - The `date-time` format is represented as `YYYY-MM-DDTHH:mm:ss.sssssssZ`.
    ...
    - Sub-second precision is optional. (such as `2024-12-01T10:00:00Z`)
    - The number of digits after the decimal point can vary from 1 to 7 (such as `2024-12-01T10:00:00.0000000Z`).

| Name           | Cardinality | Type     | Description & Constraints                                                                                                                                          |
| -------------- | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| start_date     | 0..1        | `string` | The start date of the event in RFC 3339 format (such as `2024-12-01`).                                                                                             |
| start_datetime | 0..1        | `string` | The start datetime of the event in RFC 3339 format (such as `2024-12-01T10:00:00.0000000Z`). Only UTC time is allowed.                                             |
| end_date       | 0..1        | `string` | The end date of the event in RFC 3339 format (such as `2024-12-01`). This is optional if `start_date` is used.                                                     |
| end_datetime   | 0..1        | `string` | The end datetime of the event in RFC 3339 format (such as `2024-12-01T10:00:00.0000000Z`). Only UTC time is allowed. This is optional if `start_datetime` is used. |
...
bidirectional transformation between the two formats. For example, if you drop the time from date-time, you will have a
hard time getting the time value back.
...
and an end. Event does not support conveying an ongoing event (for example, there exists a start date, but no end date
is provided because it is still current or ongoing). This is because the message specification needs to support

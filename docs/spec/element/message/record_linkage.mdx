...
Linkages shall also contain the following properties:

| Name               | Cardinality | Type     | Description & Constraints                                                                                                                                                                                        |
| ------------------ | ----------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| linked_system_id   | 1..1        | `string` | A valid [CSNR IT Service Management](https://apps.nrs.gov.bc.ca/int/jira/secure/ObjectSchema.jspa?id=1&typeId=5) code which identifies the source system, service or asset that the linked data originates from. |
...
| linked_record_kind | 1..1        | `string` | The kind of record the source system stores this linked record as (for example, Anchor or Permit).                                                                                                               |
